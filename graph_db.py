...
class CYOAGraphDB:
    ...
    def get_scene_history_path(self, current_scene_id: str):
        ...
        with self.driver.session() as session:
            # Note: This is a simplified path query. If your graph loops or has
            # multiple paths to the same node, you'll need a more specific query
            # or to track the player's exact 'Playthrough' path separately.
...
if __name__ == "__main__":
    # Ensure Neo4j docker container is running first!
    db = CYOAGraphDB()
    try:
        story_title = db.create_story_node_and_get_title("The Dark Forest Escape")
        scene1 = db.create_scene_node("You wake up in a dark forest. A path leads north.", ["Walk north towards the path."], story_title)
        scene2 = db.create_scene_node("You find a small abandoned cabin.", [], story_title)
        
        db.create_choice_edge(scene1, scene2, "Walk north towards the path.")
        
        print(f"Graph initialized with nodes {scene1} and {scene2}")
        print("Path history for scene 2:")
        print(db.get_scene_history_path(scene2))
    except Exception as e:
        print(f"Error connecting to Neo4j. Is the Docker container running? Details: {e}")
    finally:
        db.close()
